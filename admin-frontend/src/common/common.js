...
export const formatTimeMs = (timestamp, format = 'MM-DD HH:mm') => {
  if (!timestamp) return ''
...
export const formatTimeHMS = (timestamp) => {
  if (!timestamp) return ''
  const date = new Date(timestamp)
...
let refreshPromise = null      // 共享刷新 Promise 避免重复请求
...
/**
 * 获取存储的 token（优先 admin_token，支持 localStorage 和 sessionStorage）
 */
...
/**
 * 静默刷新管理员 token
 * 调用后端 /api/admin-manage/refresh-token 获取新的 access token
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      if (refreshed) {
        ...
        const retryTid = setTimeout(() => retryController.abort(), timeout)
...
/**
 * 处理 fetch response（从 request 函数中提取，便于重试复用）
 */
