      ...
      <div class="modal-content cs-modal">
        ...
        <div class="modal-body">
          <div class="form-group" v-if="!isEdit">
            <label>用户ID <span class="label-hint">（自动生成）</span></label>
            <input type="number" v-model="form.userId" placeholder="留空自动分配" class="form-input" />
          </div>
...
const openAddModal = () => {
  isEdit.value = false
  editId.value = null
  form.value = { userId: null, name: '', avatar: '', role: 'normal', description: '' }
...
const saveService = async () => {
  ...
  try {
    ...
    if (!isEdit.value && !payload.userId) {
      delete payload.userId // 留空让后端自动分配
    }
    let res
    if (isEdit.value) {
